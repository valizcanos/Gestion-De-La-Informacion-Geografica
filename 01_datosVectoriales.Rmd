...
Espacialmente, estos **objetos** se suelen representar mediante datos **vectoriales**. Este tipo de datos constan de una descripción geométrica y forma (como puntos, lineas o poligonos). Por ejemplo las fronteras de los países, estados, provincias, prefecturas, departamentos, municipios o condados estan delimitadas por polígonos.
...
srcoord <- CRS('+init=epsg:4326') #Nuestro src es WGS84, 
#código +init=epsg:4326 se refiere al código EPSG para el sistema de coordenadas geográficas WGS 84

#Además de +init, existen otros parámetros que se pueden utilizar para definir un sistema de referencia de coordenadas (CRS) en R. Algunos de los parámetros más comunes son:

# +init: Define el Sistema de Referencia de Coordenadas (CRS) basado en el EPSG
# +proj: Define la proyección utilizada por el CRS.
# +datum: Define el datum utilizado por el CRS.
# +ellps: Define el elipsoide utilizado por el CRS.
# +units: Define las unidades utilizadas por el CRS.
# +no_defs: Indica que no se deben utilizar las definiciones de CRS por defecto.
# Por ejemplo: "+proj=merc +datum=WGS84 +units=m", en este caso se definió un CRS utilizando la proyección de Mercator, el datum WGS84 y las unidades de metros
#El código EPSG es un identificador numérico único que se utiliza para identificar un sistema de coordenadas específico
...
plot(puntos,cex=dengueCasos/10, axes=TRUE, las=2)
```

Información de intrés <a href="extension://nhppiemcomgngbgdeffdgkhnkjlgpcdi/data/pdf.js/web/viewer.html?file=https%3A%2F%2Fwww.nceas.ucsb.edu%2Fsites%2Fdefault%2Ffiles%2F2020-04%2FOverviewCoordinateReferenceSystems.pdf"> Overview of Coordinate Reference Systems (CRS) in R</a>
